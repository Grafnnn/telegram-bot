...
VALIDATOR = REPO_ROOT / "scripts" / "validate_crop_only_track_b_remaining_fixtures_packet.py"
...
def test_crop_only_track_b_remaining_fixtures_packet_validator_passes() -> None:
    result = subprocess.run([sys.executable, str(VALIDATOR)], check=False, text=True, capture_output=True)

    assert result.returncode == 0, result.stderr or result.stdout
    assert "crop-only Track B remaining-fixtures packet validated" in result.stdout


def test_crop_only_track_b_remaining_fixtures_packet_is_not_execution_approval() -> None:
    packet = PACKET_MD.read_text(encoding="utf-8")
    manifest = json.loads(MANIFEST_JSON.read_text(encoding="utf-8"))

    assert manifest["status"] == "remaining_fixtures_packet_not_approved_for_execution"
...
def test_crop_only_track_b_remaining_fixtures_packet_limits_scope_to_remaining_fixtures() -> None:
    manifest = json.loads(MANIFEST_JSON.read_text(encoding="utf-8"))

    assert manifest["fixture_ids_after_go"] == [
        "pm001-pattern-boundary",
        "pm003-large-pattern-scale",
        "pm004-edge-boundary-stress",
    ]
    assert manifest["excluded_fixture_ids"] == ["pm001-solid-frontal"]
    assert manifest["expected_provider_generations_after_go"] == 3
    assert manifest["max_provider_http_requests_after_go"] == 4
    assert manifest["stop_on_first_threshold_failure_after_go"] is True


def test_crop_only_track_b_remaining_fixtures_packet_stays_crop_only_and_non_rollout() -> None:
    manifest = json.loads(MANIFEST_JSON.read_text(encoding="utf-8"))
    ...
    assert manifest["visual_quality_thresholds"]["rollout_approval_from_this_packet"] is False
