...
def test_bot_user_can_select_only_published_fabric(client: TestClient) -> None:
    ...
    select_response = client.post(f"/api/bot/users/{telegram_id}/selected-fabric", json={"fabric_id": published_id})
    assert select_response.status_code == 200, select_response.text
    assert select_response.json()["selected_fabric_id"] == published_id

    selected_response = client.get(f"/api/bot/users/{telegram_id}/selected-fabric")
    ...
    for fabric_id in [draft_id, hidden_id, archived_id]:
        forbidden_response = client.post(f"/api/bot/users/{telegram_id}/selected-fabric", json={"fabric_id": fabric_id})
        assert forbidden_response.status_code == 403, forbidden_response.text

    missing_response = client.post(f"/api/bot/users/{telegram_id}/selected-fabric", json={"fabric_id": missing_id})
...
def test_bot_selected_fabric_returns_clear_message_when_empty(client: TestClient) -> None:
    ...
    upsert_response = client.post("/api/bot/users/upsert", json={"telegram_id": telegram_id})
    assert upsert_response.status_code == 200, upsert_response.text
