...
| `TRYON_PROVIDER_STRATEGY`, `TRYON_MAX_PROVIDER_ATTEMPTS`, `TRYON_DEBUG_BUNDLE_ENABLED` | backend user-photo try-on | Full-frame edit strategy controls. Default strategy is `chatgpt_like_masked_edit`; `vision_guided_edit` is an alternative staging strategy that sends the original full-frame photo plus fabric reference without a provider mask while keeping backend guardrails mandatory. Attempts are clamped to `1..3`; debug reports are sanitized and disabled by default. |
