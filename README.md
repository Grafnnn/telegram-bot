...
| `TRYON_PROVIDER_STRATEGY`, `TRYON_MAX_PROVIDER_ATTEMPTS`, `TRYON_DEBUG_BUNDLE_ENABLED`, `TRYON_DEBUG_ARTIFACTS_ENABLED`, `TRYON_DEBUG_ARTIFACT_RETENTION_HOURS` | backend user-photo try-on | Full-frame masked edit strategy controls. Default strategy is `chatgpt_like_masked_edit`; attempts are clamped to `1..3`; debug reports/artifacts are sanitized, stored outside public `/uploads`, and disabled by default. |
