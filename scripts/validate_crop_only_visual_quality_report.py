#!/usr/bin/env python3
"""Validate redacted crop-only visual-quality expansion 003 report."""
...
def validate() -> None:
    ...
    if report.get("experiment_id") != "crop-only-visual-quality-expansion-003":
        raise AssertionError("Unexpected experiment id.")
    if report.get("decision") != "GO_FOR_MORE_CROP_ONLY_TESTING":
        raise AssertionError("Decision must be more controlled testing only.")
    if report.get("successful_provider_generations") != 4:
        raise AssertionError("Expected exactly 4 successful provider generations.")
    if report.get("total_http_requests") != 5:
        raise AssertionError("Total HTTP request count must be 5.")
    if report.get("retry_count") != 1:
        raise AssertionError("Retry count must be recorded as 1.")
    if report.get("visual_review_summary", {}).get("average_score", 0) < 4.0:
        raise AssertionError("Visual average must clear threshold.")
    if report.get("visual_review_summary", {}).get("minimum_dimension_score", 0) < 4:
        raise AssertionError("Visual minimum dimension score must clear threshold.")
    ...
    for entry in fixtures:
        ...
        if entry.get("visual_review") != "pass":
            raise AssertionError(f"{entry.get('fixture_id')} visual review must pass.")
        ...
        if entry.get("minimum_visual_dimension_score", 0) < 4:
            raise AssertionError(f"{entry.get('fixture_id')} visual score below threshold.")
    ...
    required_markdown = [
        ...
        "Any next provider/model/prompt expansion",
    ]
...
def main() -> int:
    try:
        ...
        return 1
    print("crop-only visual quality expansion 003 report validated")
    return 0
