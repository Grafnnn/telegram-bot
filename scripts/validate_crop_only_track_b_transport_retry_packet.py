#!/usr/bin/env python3
"""Validate Track B transport retry packet."""
...
EXPECTED_FIXTURE_ID = "pm001-solid-frontal"
...
def validate() -> None:
    ...
    if parent_report.get("decision") != "HOLD_TRACK_B_OPERATOR_REVIEW":
        raise AssertionError("Parent Track B report must be HOLD.")
    if parent_report.get("stop_condition") != "provider_call_failed:URLError":
        raise AssertionError("Parent Track B stop condition must remain transport-focused.")
    if parent_report.get("actual_provider_http_requests") != 2:
        raise AssertionError("Parent Track B report must record two HTTP requests.")
    if parent_report.get("fixtures") != []:
        raise AssertionError("Parent Track B report must have no completed fixtures.")

    if manifest.get("manifest_id") != "crop-only-track-b-transport-retry-packet-008":
        raise AssertionError("Unexpected manifest id.")
    if manifest.get("status") != "transport_retry_packet_not_approved_for_execution":
        raise AssertionError("Manifest must remain not approved for execution.")
    if manifest.get("baseline_commit") != "c343ca41f410f816dcfeeedc1575748317b81ffc":
        raise AssertionError("Baseline mismatch.")
    if manifest.get("future_target_after_explicit_go") != "local/dev provider transport retry":
    ...
    if manifest.get("fixture_ids_after_go") != [EXPECTED_FIXTURE_ID]:
        raise AssertionError("Transport retry must be limited to one fixture.")
    if manifest.get("expected_provider_generations_after_go") != 1:
        raise AssertionError("Expected provider generations must be 1.")
    if manifest.get("max_provider_http_requests_after_go") != 2:
...
def main() -> int:
    try:
        ...
        return 1
    print("crop-only Track B transport retry packet validated")
    return 0
